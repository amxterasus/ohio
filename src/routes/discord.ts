...
discordRouter.get("/callback", async (c) => {
  ...
  try {
    ...
    // 2. Llama a la API de Discord con el token para obtener datos del usuario
    ...
    // 👉 Ya tenés los datos del usuario:
    // user.id, user.username, user.avatar, etc.
    console.log(user);

    // Podés guardar esto en una sesión, cookie, etc.
    return c.json({ user });
  } catch (error) {
    if (
