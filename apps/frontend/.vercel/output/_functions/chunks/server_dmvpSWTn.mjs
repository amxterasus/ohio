...
import {
  K as decryptString,
  O as createSlotValueFromString,
  P as isAstroComponentFactory,
  r as renderComponent,
  a as renderTemplate,
  z as REROUTE_DIRECTIVE_HEADER,
  A as AstroError,
  Q as i18nNoLocaleFoundInPath,
  S as ResponseSentError,
  T as MiddlewareNoDataOrNextCalled,
  V as MiddlewareNotAResponse,
  W as originPathnameSymbol,
  X as RewriteWithBodyUsed,
  Y as GetStaticPathsRequired,
  Z as InvalidGetStaticPathsReturn,
  _ as InvalidGetStaticPathsEntry,
  $ as GetStaticPathsExpectedParams,
  a0 as GetStaticPathsInvalidRouteParam,
  a1 as PageNumberParamNotFound,
  D as DEFAULT_404_COMPONENT,
  a2 as NoMatchingStaticPathFound,
  a3 as PrerenderDynamicEndpointPathCollide,
  a4 as ReservedSlotName,
  a5 as renderSlotToString,
  a6 as renderJSX,
  a7 as chunkToString,
  a8 as isRenderInstruction,
  a9 as ForbiddenRewrite,
  aa as SessionStorageInitError,
  ab as SessionStorageSaveError,
  y as ROUTE_TYPE_HEADER,
  ac as ASTRO_VERSION,
  ad as CspNotEnabled,
  ae as LocalsReassigned,
  af as PrerenderClientAddressNotAvailable,
  C as clientAddressSymbol,
  ag as ClientAddressNotAvailable,
  ah as StaticClientAddressNotAvailable,
  ai as AstroResponseHeadersReassigned,
  J as responseSentSymbol$1,
  aj as renderPage,
  ak as REWRITE_DIRECTIVE_HEADER_KEY,
  al as REWRITE_DIRECTIVE_HEADER_VALUE,
  am as renderEndpoint,
  f as ActionCalledFromServerError,
} from './astro/server_ZzAh1HeT.mjs';
import {
  a as appendForwardSlash,
  j as joinPaths,
  r as removeTrailingForwardSlash,
  p as prependForwardSlash,
  t as trimSlashes,
} from './path_BuZodYwm.mjs';
...
import {
  ...
  h as callSafely,
  A as ActionError,
  i as ActionInputError,
  s as serializeActionResult,
  j as ACTION_RPC_ROUTE_PATTERN,
  b as ACTION_QUERY_PARAMS,
} from './astro-designed-error-pages_BuexQOyv.mjs';
...
function matchRoute(pathname, manifest) {
  return manifest.routes.find((route) => {
    return (
      route.pattern.test(pathname) || route.fallbackRoutes.some((fallbackRoute) => fallbackRoute.pattern.test(pathname))
    );
  });
}
...
function generatePaginateFunction(routeMatch, base) {
  return function paginateUtility(data, args = {}) {
    ...
    const result = [...Array(lastPage).keys()].map((num) => {
      ...
      const last =
        pageNum === lastPage ? void 0 : addRouteBase(routeMatch.generate({ ...params, page: String(lastPage) }), base);
      return {
        params,
        props: {
          ...additionalProps,
...
async function callGetStaticPaths({ mod, route, routeCache, logger, ssr, base }) {
...
function getParams(route, pathname) {
  ...
  const paramsMatch =
    ...
    route.fallbackRoutes.map((fallbackRoute) => fallbackRoute.pattern.exec(pathname)).find((x) => x);
...
class AstroSession {
  ...
  constructor(cookies, { cookie: cookieConfig = DEFAULT_COOKIE_NAME, ...config }, runtimeMode) {
    const { driver } = config;
    if (!driver) {
      throw new AstroError({
  ...
  async #ensureStorage() {
    ...
    if (!driver) {
      throw new AstroError({
        ...SessionStorageInitError,
        message: SessionStorageInitError.message('The module did not export a driver.', this.#config.driver),
      });
    }
    try {
...
class RenderContext {
  ...
  async render(componentInstance, slots = {}) {
    ...
    const lastNext = async (ctx, payload) => {
      ...
      switch (this.routeData.type) {
        case 'endpoint': {
          response2 = await renderEndpoint(componentInstance, ctx, this.routeData.prerender, logger);
          break;
        }
        case 'redirect':
  ...
  createAstro(result, astroStaticPartial, props, slotValues, apiContext) {
    ...
    if (this.isRewriting) {
      astroPagePartial = this.#astroPagePartial = this.createAstroPagePartial(result, astroStaticPartial, apiContext);
    } else {
      astroPagePartial = this.#astroPagePartial ??= this.createAstroPagePartial(result, astroStaticPartial, apiContext);
    }
...
function defineAction({ accept, input: inputSchema, handler }) {
  const serverHandler =
    accept === 'form' ? getFormServerHandler(handler, inputSchema) : getJsonServerHandler(handler, inputSchema);
  ...
  if (callerInfo && context.request.method === 'POST' && !actionResultAlreadySet) {
    action = {
      ...
      handler: async () => {
        ...
        const callerInfoName = shouldAppendForwardSlash(pipeline.manifest.trailingSlash, pipeline.manifest.buildFormat)
        ...
        const actionAPIContext = Object.create(
          ...
          Object.fromEntries(
            Object.entries(Object.getOwnPropertyDescriptors(context)).filter(([key]) => !omitKeys.includes(key))
          )
        );
...
export {
  ...
  defineAction as d,
  normalizeTheLocale as e,
  defineMiddleware as f,
  getActionContext as g,
  SERVER_ISLAND_ROUTE as h,
  isRequestServerIsland as i,
  createEndpoint as j,
  findRouteToRewrite as k,
  RenderContext as l,
  matchRoute as m,
  notFound as n,
  getSetCookiesFromResponse as o,
